...
def visualizate_save(model, IMG_SIZE):
    ...
    for class_dir in os.listdir(directory_test):
        ...
        for imname in os.listdir(os.path.join(directory_test,class_dir)):
            ...
            icam = GradCAM(model, out, 'conv5_block16_2_conv') 
...
def model(params):
    ...
    BEST_MODEL_FNAME = f'/ghome/group06/lab3_C3_G06/weights/best_weights_0.3_0.25.keras'

    # create the base pre-trained model
    # base_model = VGG16(weights='imagenet', include_top=False)
    input = tf.keras.Input(shape=(IMG_WIDTH, IMG_HEIGHT, 3))
    base_model = DenseNet121(weights='imagenet', include_top=False, input_tensor=input)
...
params = {
    ...
    'lr': 0.046,  # 0.0001, 0.001, 0.01, 0.1, 0.2, 0.3
